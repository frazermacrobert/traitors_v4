...
function rosterHas(id){ return !!S.allEmployees.find(e => e.id === id); }

function getDefaultYouId(){
  try {
    const last = localStorage.getItem('lastCharacterId');
    if (last && rosterHas(last)) return last;
  } catch {}
  return S.allEmployees[0]?.id || null;
}

// Ensure UI and preview reflect a valid default selection
function ensureDefaultCharacterSelected(){
  const sel = document.getElementById('playerSelect');
  if (!sel) return;

  // If empty or invalid, set to a safe default
  if (!sel.value || !rosterHas(sel.value)){
    const def = getDefaultYouId();
    if (def){
      sel.value = def;
      const img = document.getElementById('character-preview-image');
      if (img) img.src = `assets/pngs/${def}.png`;
    }
  }
}
...
async function loadData(){
  ...
  ensureDefaultCharacterSelected(); // ensure a valid default is visible and ready
}
...
window.addEventListener('DOMContentLoaded', async ()=>{
  ...
  document.getElementById('startGameBtn').onclick = () => {
    // Guarantee the select has a valid value
    ensureDefaultCharacterSelected();

    let you = playerSelect.value;
    if (!you) you = getDefaultYouId(); // final fallback

    if (!you){
      alert("No characters available to start a game.");
      return;
    }
    ...
    // remember the last pick so next session keeps it
    try { localStorage.setItem('lastCharacterId', you); } catch {}
  ...
  document.getElementById('confirmCharacterBtn').onclick = () => {
    ensureDefaultCharacterSelected(); // make sure something is picked
  ...
  playerSelect.onchange = (event) => {
    ...
    if (selectedPlayer) {
        ...
        try { localStorage.setItem('lastCharacterId', selectedPlayer.id); } catch {}
    }
  };
